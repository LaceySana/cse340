...
-- 6 - Update image and thumbnail urls
UPDATE public.inventory
...
(
    ...
    msg_timestamp timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
    sender_id integer NOT NULL,
    conversation_id integer NOT NULL,
    CONSTRAINT msg_pkey PRIMARY KEY (msg_id)
);

-- Table structure for `conversation` table
CREATE TABLE IF NOT EXISTS public.conversation
(
    conversation_id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    subject character varying(255),
    CONSTRAINT conversation_pkey PRIMARY KEY (conversation_id)
);

-- Table structure for `message_recipient` table
CREATE TABLE IF NOT EXISTS public.message_recipient
(
    account_id integer NOT NULL,
    msg_id integer NOT NULL,
    was_read boolean DEFAULT false,
    deleted_at timestamptz DEFAULT NULL,
    CONSTRAINT message_recipient_pkey PRIMARY KEY (account_id, msg_id)
);

-- Create relationship between `message` and `conversation` tables
...
-- Create relationship between `message` and `message_recipient` tables
ALTER TABLE IF EXISTS public.message_recipient
    ADD CONSTRAINT fk_mr_acct FOREIGN KEY (account_id)
    REFERENCES public.account (account_id) MATCH SIMPLE
    ON UPDATE CASCADE
    ON DELETE CASCADE;
...
-- Create relationship between `message` and `message_recipient` tables
ALTER TABLE IF EXISTS public.message_recipient
    ADD CONSTRAINT fk_mr_msg FOREIGN KEY (msg_id)
    REFERENCES public.message (msg_id) MATCH SIMPLE
    ON UPDATE CASCADE
    ON DELETE CASCADE;